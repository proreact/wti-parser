...
try {
  ...
  if (error.toString().includes('Unexpected end of JSON input')) {
    config = 'Invalid JSON'
  } else if (error.toString().includes('Cannot find module')) {
    config = 'No config'
  }
}
...
module.exports = {
  ...
  validateConfig: function () {
    switch (config) {
      case 'No config': {
        this.handleError({
          error: 'There is no wti_config.json configuration file in this folder.',
        })
        process.exit(1)
        break
      }
  ...
  createConfig: function (newConfig) {
    ...
    this.get('.json', newConfig.projectKey).then((result) => {
      //Add project to config
      configObj.projects['project_' + result.project.name.replaceAll(/[\s-_]/g, '').toLowerCase()] =
        {
      ...
      fs.outputFile(
        ...
        (err) => {
          if (err) {
            ...
            console.log(
              `Config file written successfuly with project "${result.project.name}"`.green,
            )
          }
        },
      )
  ...
  put: function (url = '', data) {
    ...
    return new Promise((resolve, reject) => {
      request.put(
        { url: `https://webtranslateit.com/api/projects/${apiKey}${url}`, formData: data },
        (err, resp, body) => {
          let parsedBody = body.trim() ? JSON.parse(body) : {}
