...
module.exports = {
    ...
    Utils.get('/stats.json')
      .then((result) => {
        ...
        for (const l of locales) {
          let percTranslated = (
            ((result[l].count_strings - result[l].count_strings_to_translate) * 100) /
            result[l].count_strings
          ).toFixed(2)
  ...
  getProject: function () {
    return new Promise((resolve, reject) => {
      Utils.get('.json').then(resolve).catch(reject)
    })
  },
  ...
  add: function (key = '') {
    ...
    Utils.get('.json', key).then((result) => {
      ...
      config.projects['project_' + result.project.name.replaceAll(/[\s-_]/g, '').toLowerCase()] = {
