...
program
  .command('pull')
  .description(
    'Pull language files. If no locales are passed, we pull all locales except the master one.',
  )
  .option('-p, --project [project]', 'Project key from config you want to use command for')
...
program
  .command('push')
  .description(
    'Pull language files. If no locales are passed, we push all locales except the master one.',
  )
  .option('-p, --project [project]', 'Project key from config you want to use command for')
...
function selectProject(cb, specifiedProject = false) {
  ...
  if (specifiedProject) {
    if (projects.includes(specifiedProject)) {
...
function initConfig(cb) {
  let prompts = [
    {
      ...
      name: 'projectKey',
      validate: (input) => {
        return !input || input.trim().length === 0 ? 'You need to provide an API KEY!' : true
      },
    },
    {
  ...
  ask.prompt(prompts).then((answer) => {
    cb(answer)
  })
}
...
function addProject(key, cb) {
  ...
  if (!key) {
    prompts.push({
      ...
      name: 'key',
      validate: (input) => {
        return !input || input.trim().length === 0 ? 'You need to add an API KEY!' : true
      },
    })
  }
